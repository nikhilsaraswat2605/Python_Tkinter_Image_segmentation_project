...
def fileClick(clicked, dataset, segmentor):
    ...
    filepath = filedialog.askopenfilename(filetypes=[('Jpg Files', '*.jpg'), ('jpeg Files', '*.jpeg')], initialdir="data/imgs/")
    print(filepath)
    ...
    try:
        ...
        print("******************* ", err, " *******************")
        msg.configure(font=("Arial Bold", 15), text='No image is selected !', bg="#FFEDDB")
        msg.place(x=80, y=25)
        return
    ...
    print("done!")
    msg.configure(font=("Arial Bold", 15), text=f"""{path.basename(img_path["path"])} - image is selected !""", bg="#FFEDDB")
    msg.place(x=80, y=25)
    e.delete(0, 'end')
...
if __name__ == '__main__':
    ...
    style.configure("TCombobox", fieldbackground="#FFEDDB", background="#FFEDDB")
    ####### CODE REQUIRED (END) #######
    ...
    msg = Label(root, text="", justify=CENTER, bg="#FFEDDB")  # labeling the message
    ...
    selectButton = Button(text='. . .', command=partial(fileClick, clicked, dataset, segmentor), padx=5, bg="#EDBB99")
    ####### CODE REQUIRED (END) #######
    ####### CODE REQUIRED (START) #######
    # Declare the drop-down button
    clicktypeDropDown = ttk.Combobox(root, width=27, values=options, textvariable=clicked, state="readonly")
    quitbutton = Button(text='QUIT', command=root.destroy, padx=5, bg="#EDBB99")
    ####### CODE REQUIRED (END) #######
    # This is a `Process` button, check out the sample video to know about its functionality
    logo = ImageTk.PhotoImage(file='./logo/kgplogo.jpg') # setting a logo for GUI
    # Setting icon of master window
    root.iconphoto(False, logo)
    myButton = Button(root, text="Process", command=partial(process, clicked), padx=2, bg="#EDBB99")
    ...
    quitbutton.grid(row=0, column=8, columnspan=2, padx=5)
